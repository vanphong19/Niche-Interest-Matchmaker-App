...
/// Model cho thành viên phòng chat
class ChatMemberModel {
  ...
  factory ChatMemberModel.fromJson(Map<String, dynamic> j) => ChatMemberModel(
    ...
    role: (j['role'] ?? j['Role'] ?? 'Member').toString(),
    joinedAt:
        DateTime.tryParse(
          j['joinedAtUtc']?.toString() ?? j['JoinedAtUtc']?.toString() ?? '',
        ) ??
        DateTime.now(),
    isOnline: j['isOnline'] as bool? ?? j['IsOnline'] as bool? ?? false,
    isFriend: j['isFriend'] as bool? ?? j['IsFriend'] as bool? ?? false,
    friendshipStatus: (j['friendshipStatus'] ?? j['FriendshipStatus'] ?? 'None')
        .toString(),
  );
...
class ChatApiService {
  ...
  Future<List<ChatRoomModel>> getMyRooms({String? currentUserId}) async {
    try {
      final response = await _dio.get('/api/chat-rooms/my');
      ...
      debugPrint(
        'getMyRooms error: ${e.response?.statusCode} ${e.response?.data}',
      );
      rethrow;
    }
  }
  ...
  Future<List<ChatRoomModel>> getChatRoomsByEvent(String eventId) async {
    try {
      ...
      debugPrint(
        'getChatRoomsByEvent error: ${e.response?.statusCode} ${e.response?.data}',
      );
      rethrow;
    }
  }
  ...
  Future<ChatRoomModel> createGroupChatRoom(
    ...
    try {
      ...
      debugPrint(
        'createGroupChatRoom error: ${e.response?.statusCode} ${e.response?.data}',
      );
      rethrow;
    }
  }
  ...
  Future<ChatRoomModel> getOrCreatePrivateChat(
    ...
    try {
      ...
      debugPrint(
        'getOrCreatePrivateChat error: ${e.response?.statusCode} ${e.response?.data}',
      );
      rethrow;
    }
  }
  ...
  Future<List<ChatMessageModel>> getMessages(
    ...
    try {
      ...
      final unwrapped = _unwrap(response.data);
      debugPrint(
        'ChatApiService.getMessages unwrapped type: ${unwrapped.runtimeType}',
      );
      ...
      if (unwrapped is Map<String, dynamic>) {
        items =
            (unwrapped['items'] ?? unwrapped['Items'] ?? []) as List<dynamic>;
      ...
      debugPrint(
        'getMessages error: ${e.response?.statusCode} ${e.response?.data}',
      );
      rethrow;
    }
  }
  ...
  Future<ChatRoomModel> openDirectMessage(String targetUserId) async {
    try {
      final response = await _dio.post(
        '/api/users/$targetUserId/direct-message',
      );
      final unwrapped = _unwrap(response.data);
      ...
      debugPrint(
        'openDirectMessage error: ${e.response?.statusCode} ${e.response?.data}',
      );
      rethrow;
    }
  }
  ...
  Future<String> uploadChatImage(String filePath) async {
    try {
      ...
      final unwrapped = _unwrap(response.data);
      final imageUrl = unwrapped is Map
          ? (unwrapped['imageUrl'] ?? unwrapped['ImageUrl'])?.toString()
          : null;
      ...
      debugPrint(
        'uploadChatImage error: ${e.response?.statusCode} ${e.response?.data}',
      );
      rethrow;
    }
  }
  ...
  Future<List<ChatMemberModel>> getRoomMembers(String chatRoomId) async {
    try {
      final response = await _dio.get('/api/chat-rooms/$chatRoomId/members');
      final unwrapped = _unwrap(response.data);
      if (unwrapped is List) {
        return unwrapped
      ...
      debugPrint(
        'getRoomMembers error: ${e.response?.statusCode} ${e.response?.data}',
      );
      rethrow;
    }
  }
