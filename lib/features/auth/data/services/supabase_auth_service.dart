...
///
/// Flow: GoogleSignIn SDK → get idToken → Supabase signInWithIdToken
///       → returns user info map for backend social login endpoint.
class SupabaseAuthService {
  ...
  GoogleSignIn get _googleSignIn {
    return GoogleSignIn(
      ...
      serverClientId: '31146531682-o6qjjahltbfbnsh94504sialqcpsb6m9.apps.googleusercontent.com',
    );
  }
    ...
    try {
      ...
      if (kIsWeb) {
        await _supabase.auth.signInWithOAuth(
          ...
          redirectTo: Uri.base.origin, // Redirect về đúng port hiện tại của Flutter Web thay vì Site URL mặc định của Supabase
        );
      ...
      // 2. On Mobile (iOS/Android), use Google Sign-In SDK for native popup
      // Gọi signOut trước để xóa cache kẹt (Fix lỗi popup bật lên rồi tắt luôn)
      try { await _googleSignIn.signOut(); } catch (_) {}
      
      final GoogleSignInAccount? googleUser = await _googleSignIn.signIn();
      if (googleUser == null) {
        throw Exception('Google Sign-In trả về null (Lỗi cấu hình hoặc người dùng hủy)');
      }
